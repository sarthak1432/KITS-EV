    ...
    >
      ...
      >
        ...
        <div className="relative h-full sm:max-h-[calc(100dvh-6rem)] overflow-y-auto p-6 sm:p-8">
            ...
            {scooterCatalog.map((group, index) => {
              ...
              return (
                <button
                  ...
                  className={`reveal relative h-40 sm:h-64 overflow-hidden rounded-[1.8rem] sm:rounded-[2.5rem] border transition-all duration-700 hover:-translate-y-2 hover:shadow-[0_45px_100px_-20px_rgba(15,23,42,0.18)] active:scale-[0.97] ${isActive
                ...
                >
                  ...
                  <div className="absolute inset-x-0 bottom-4 sm:bottom-6 flex justify-center px-4">
                    <div className={`rounded-full px-4 py-1.5 sm:px-5 sm:py-2 backdrop-blur-md border shadow-sm transition-all duration-500 ${isActive ? 'bg-slate-950 text-white border-white/20' : 'bg-white/20 border-white/40 hover:bg-slate-950 hover:text-white'}`}>
                      <p className="text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.2em] sm:tracking-[0.3em] font-display">{group.brand}</p>
                    </div>
                  </div>
                </button>
              )
          ...
          {selectedGroup && (
            <>
              <div className="mt-6 sm:mt-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 sm:gap-6 rounded-[1.8rem] sm:rounded-[2rem] border border-white/90 bg-white/82 px-6 py-5 sm:px-8 sm:py-6 shadow-[0_20px_50px_rgba(148,163,184,0.15)] backdrop-blur-xl">
                <div className="flex items-center gap-4 sm:gap-6">
                  {/* Perfectly Attached Brand Identifier (Product View) */}
                  <div className="h-14 w-14 sm:h-20 sm:w-20 shrink-0 overflow-hidden rounded-xl sm:rounded-2xl border border-slate-200 bg-white p-1 shadow-sm sm:p-1.5">
                    <img 
              ...
              <div className="mt-8 grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                {selectedGroup.models.map((model, index) => (
                  <a
                    ...
                    className="reveal group relative h-48 sm:h-72 overflow-hidden rounded-[1.8rem] sm:rounded-[2.5rem] border border-white bg-white/40 transition-all duration-700 hover:-translate-y-2 hover:shadow-[0_45px_100px_-20px_rgba(15,23,42,0.18)]"
                  >
                    <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,_rgba(255,255,255,1)_0%,_rgba(255,255,255,0.4)_70%,_transparent_100%)] opacity-80" />
                    ...
                    <div className="absolute inset-x-0 bottom-4 sm:bottom-6 flex justify-center px-4">
                      <div className="flex items-center gap-2 rounded-full bg-white/20 px-4 py-1.5 sm:px-6 sm:py-2.5 backdrop-blur-md border border-white/40 shadow-sm transition-all duration-500 group-hover:bg-slate-950 group-hover:text-white">
                        <svg className="h-3 w-3 sm:h-3.5 sm:w-3.5 transition-colors group-hover:text-emerald-400" viewBox="0 0 24 24" fill="currentColor">
                          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                        </svg>
